...
class Runner(object):
    ...
    def _run_test(self, test_dict):
        ...
        # variables_af (variables after extract)
        variables_af = test_dict.get("variables_af", {})
        if variables_af:
            logger.log_debug("start to process variables_af after extract.")
            logger.log_debug("Available variables before variables_af:")
            logger.log_debug("  session_variables: {}".format(
                dict(list(self.session_context.session_variables_mapping.items())[:10])
            ))
            logger.log_debug("  test_variables: {}".format(
                dict(list(self.session_context.test_variables_mapping.items())[:10])
            ))
            
            # 确保 test_variables_mapping 包含最新的 session_variables
            self.session_context.test_variables_mapping.update(self.session_context.session_variables_mapping)
            
            variables_af_mapping = utils.ensure_mapping_format(variables_af)
            for var_name, var_value in variables_af_mapping.items():
                logger.log_debug("Processing variables_af: {} = {}".format(var_name, var_value))
                var_value_eval = self.session_context.eval_content(var_value)
                logger.log_debug(
                    "variables_af assignment: {} = {} => {} ({})".format(
                        var_name, var_value, var_value_eval, type(var_value_eval).__name__
                    )
                )
                # 同时更新 test_variables 和 session_variables，确保 output 可以访问
                self.session_context.update_test_variables(var_name, var_value_eval)
                self.session_context.session_variables_mapping[var_name] = var_value_eval
                logger.log_debug("Variable {} successfully set to: {}".format(var_name, var_value_eval))
