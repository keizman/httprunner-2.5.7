...
""".format(script)
    ...
    def validate(self, validators):
        ...
        for validator in validators:
            ...
            # 添加详细的变量值日志
            logger.log_debug("Validator detailed info:")
            logger.log_debug("  check_item (raw): {}".format(check_item))
            logger.log_debug("  check_value (evaluated): {} ({})".format(check_value, type(check_value).__name__))
            logger.log_debug("  expect_item (raw): {}".format(expect_item))
            logger.log_debug("  expect_value (evaluated): {} ({})".format(expect_value, type(expect_value).__name__))
            logger.log_debug("  current test_variables_mapping: {}".format(
                dict(list(self.session_context.test_variables_mapping.items())[:5])  # 只显示前5个避免日志过长
            ))
